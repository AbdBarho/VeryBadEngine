import EventManagerSignleton, { EventManager } from "../../services/eventmanager";
...
export default class InputManager  {
  ...
  constructor(renderer: Renderer) {
    this.canvasParams = renderer.parameters;
    this.initListeners();
  }
  ...
  keyboardPress(e: KeyboardEvent, isPressed: boolean) {
    ...
    EventManagerSignleton.trigger("input_" + e.type + "_" + name);
  }

  mousePress(e: MouseEvent, isPressed: boolean) {
    let name = "Mouse" + (e.button + 1);
    this.updateButtonsState(name, isPressed);
    EventManagerSignleton.trigger("input_" + e.type + "_" + name, this.mousePos);
  }
  ...
  mouseStateUpdate(e: MouseEvent) {
    ...
    EventManagerSignleton.trigger("input_" + e.type, this.mousePos);
  }
  ...
  clearAll() {
    for (let key of Object.keys(this.buttonStates)) {
      ...
      if (key.indexOf("Mouse") === 0)
        EventManagerSignleton.trigger("input_mouseup", this.mousePos, key);
      else
        EventManagerSignleton.trigger("input_keyup", key);
  ...
  pixelToUnit(x: number, y: number) {
    ...
    pos.subVec(this.canvasParams.SHIFT).divVec(this.canvasParams.SCALE).floor();
    return pos;
  }
}